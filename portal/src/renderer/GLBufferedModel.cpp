...
GLBufferedModel::GLBufferedModel()
    : GLBufferObject()
{
}

GLBufferedModel::MeshInfo::MeshInfo()
    : buffers{{0, 0, 0, 0, 0}}
    , materialHash(0)
    , vao(0)
...
GLBufferedModel::MeshInfo::MeshInfo(MeshInfo&& other)
...
GLBufferedModel::MeshInfo& GLBufferedModel::MeshInfo::operator=(MeshInfo&& other)
{
    buffers = std::move(other.buffers);
...
void GLBufferedModel::loadFromAsset(std::shared_ptr<const asset_type>& asset, GLResourceManager& resourceManager)
{
   ...
   for (const auto& materialPtr: asset->materials)
   {
       ...
       resourceManager.getByAsset<GLBufferedMaterial>(d);
   }

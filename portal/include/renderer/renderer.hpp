...
public:
    ...
    void initialize(SDL_Window* window, SDL_GLContext context, assets::data_store& assetManager);
    void prepareFrame(threading::thread_pool& threadPool, SDL_Window* window, SDL_GLContext context);
    void render(const scene::scene_graph& scene, RenderResults& results);

    std::unique_ptr<ResourceManager> resourceManager;
private:
    void renderModel(const resources::Model& model, RenderState& state) const;
    void renderNode(const scene::scene_node& node, RenderState& state) const;
